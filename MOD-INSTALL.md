# Cuckoo Code 0.5.3 mod 安装到 0.5.1 记录

> 安装日期：2026-09-20
> 目标项目：C:\Users\Administrator\Desktop\cuckoo-code-0.5.1（原版本 0.5.1）
> mod 来源：H:\cuckoo\Cuckoo二次开发归档-0.5.3（基线为 0.5.3 的二次开发归档）
...
备份：原始文件已存于 H:\cuckoo\_backup051（src / tools / assets / package.json）。
...
3. **无 watchdog / retry-engine**：intercept-observer 只加 cuckoo-ai-start 监听与超时兜底，
...
- **键值链路**：cuckoo-auto-inject（开关）、cuckoo-last-project-dir（目录记忆）写入与读取一致。

## 六、运行方式

```powershell
$env:Path = "D:\NodeJS;" + $env:Path
cd C:\Users\Administrator\Desktop\cuckoo-code-0.5.1
npm start
# 或 electron .
```
...
- 长期记忆存于 `<userData>/cuckoo-memory.json`，技能存于 `<userData>/cuckoo-skills.json`。
- userData = C:\Users\Administrator\AppData\Roaming\cuckoo-ai-pro-session。
...
如需还原到安装前状态，用备份覆盖：
```powershell
Copy-Item -Recurse -Force H:\cuckoo\_backup051\src   C:\Users\Administrator\Desktop\cuckoo-code-0.5.1\src
Copy-Item -Recurse -Force H:\cuckoo\_backup051\tools C:\Users\Administrator\Desktop\cuckoo-code-0.5.1\tools
Copy-Item -Force H:\cuckoo\_backup051\package.json   C:\Users\Administrator\Desktop\cuckoo-code-0.5.1\package.json
```
（并删除新增的 9 个文件）
