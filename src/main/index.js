...
try {
  ...
  console.error('[Cuckoo Code] 初始化记忆存储失败:', err.message);
}
...
try {
  ...
  console.error('[Cuckoo Code] 初始化技能存储失败:', err.message);
}
...
function createWindow(profile) {
  ...
  const saveWindowBounds = () => {
    ...
    try {
      ...
      console.error('[Cuckoo Code] 保存窗口大小失败:', err.message);
    }
  };
  ...
  mainWindow.on('close', (event) => {
    ...
    if (global.__cuckooQuitting) return; // 真正退出时放行
...
if (!gotSingleInstanceLock) {
  ...
  app.whenReady().then(() => {
    ...
    global.__cuckooCreateWindow = () => createWindow(null);
