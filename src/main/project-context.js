...
async function initProject(skipPrompt = false, windowContext = null, presetDir = null, isCompaction = false) {
  ...
  try {
    ...
    console.log('[Cuckoo Code] 记忆注入块长度:', memorySection.length);
  } catch (err) {
    console.error('[Cuckoo Code] 构建记忆注入块失败:', err.message);
  }
  ...
  try {
    ...
    console.log('[Cuckoo Code] 技能注入块长度:', skillSection.length);
  } catch (err) {
    console.error('[Cuckoo Code] 构建技能注入块失败:', err.message);
  }
