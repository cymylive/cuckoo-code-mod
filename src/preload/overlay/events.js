...
function saveAutoInjectSetting() {
  const el = document.getElementById('cuckoo-auto-inject');
  try {
...
function bindEvents() {
  ...
  // 新会话自动注入开关：恢复状态 + 保存
  const autoInjectEl = document.getElementById('cuckoo-auto-inject');
  if (autoInjectEl) {
    autoInjectEl.checked = localStorage.getItem('cuckoo-auto-inject') === '1';
