 ...
 * 之后目录被记住（localStorage: cuckoo-last-project-dir），新会话自动复用。
 *
 * 开关：localStorage: cuckoo-auto-inject（'1' 开启）
...
/** 是否开启自动注入 */
function isEnabled() {
  try { return localStorage.getItem('cuckoo-auto-inject') === '1'; } catch (_) { return false; }
}

/** 取最近记住的项目目录（无则 null） */
function getLastDir() {
  try {
    const d = localStorage.getItem('cuckoo-last-project-dir');
...
function maybeInject() {
  ...
  console.log('[Cuckoo AutoInject] 新会话 ' + sid + '，自动注入记忆+技能（目录=' + dir + '）');
  try {
    window.electronAPI.initProject(dir, false);
  } catch (err) {
    console.error('[Cuckoo AutoInject] 自动注入失败:', err && err.message);
  }
}
...
function startAutoInjectWatcher() {
  ...
  console.log('[Cuckoo AutoInject] 新会话自动注入监视已启动（基线会话=' + lastInjectedSessionId + '）');
}
